...
# Настройка порта. Замените 'COM3' на соответствующий порт вашего Arduino
#SERIAL_PORT = 'COM3'
BAUD_RATE = 9600
...
class JoystickApp(ctk.CTk):
    def __init__(self):
        ...
        self.__is_stop_thread_keys = True
    ...
    def manage_keys(self, *args):
        for key in args:
            pyautogui.keyDown(key)
        while self.__is_stop_thread_keys == False:
            pass
        for key in args:
            pyautogui.keyUp(key)            
        ...
        try:
            ...
            while self.__is_stop_thread_read_serial_game == False:
                if ser.in_waiting > 0:
                    ...
                    while line == Readline(ser).readline().decode('utf-8').strip() and ser.in_waiting > 0:
                        pass
